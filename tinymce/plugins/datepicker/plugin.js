...
(function (tinymce, $) {
    ...
    function DatePicker() {
        ...
        this._mainDivId = "tinymvc-ui-datepicker-div"; // The ID of the main datepicker division
    ...
    $.extend(DatePicker.prototype, {
        ...
        _adjustDate: function (id, offset, period) {
            var target = $(id),
                inst = this._getInst(this.target);

            if (this._isDisabledDatepicker(this.target)) {
                return;
            }
        ...
        _selectDay: function (id, month, year, td) {
            ...
            if ($(td).hasClass(this._unselectableClass) || this._isDisabledDatepicker(this.target)) {
                return;
            }
        ...
        _selectDate: function (id, dateStr) {
            var inst = this._getInst(this.target);
            ...
            this.editor.focus();
            var selection = this.editor.dom.select('span#datepicker')[0];
            this.editor.dom.remove(selection);
            this.editor.execCommand('mceInsertContent', false, '<span class="hash-date" data-date="' + dateStr + '">' + dateStr + '</span>&nbsp;');
        ...
        renderInput: function () {
            var rawHtml = '<span id="datepicker">' +
                                '<span id="datepicker-trigger">' + this.options.trigger + '</span>' +
                                '<span id="datepicker-datetext"><span class="dummy">\uFEFF</span></span>' +
                            '</span>';

            this.editor.execCommand('mceInsertContent', false, rawHtml);
            this.editor.focus();

            this.target = this.editor.selection.dom.select('span#datepicker-datetext span')[0];

            this.editor.selection.select(this.target);
            this.editor.selection.collapse(0);

            if (!this.target.id) {
                $datepicker.uuid += 1;
                this.target.id = "dp" + $datepicker.uuid;
            }

            var inst = $datepicker._newInst($(this.target), false),
                isFixed, offset, showAnim, duration;

            inst.settings = $.extend({}, this.options || {});

            $datepicker._setDateFromField(inst);
            this._autoSize(inst);

            $.data(this.target, 'datepicker', inst);

            if (!$datepicker._pos) { // position below input
                $datepicker._pos = $datepicker._findPos(this.target);
                $datepicker._pos[1] += this.target.offsetHeight; // add the height
            }
            ...
            offset = this.editor.inline ? this.offsetInline() : this.offset();;
            $datepicker._pos = null;
            //to avoid flashes on Firefox
            inst.dpDiv.empty();
            // determine sizing offscreen
            ...
            if (!inst.inline) {
                showAnim = $datepicker._get(inst, "showAnim");
                duration = $datepicker._get(inst, "duration");
                //inst.dpDiv.css("z-index", datepicker_getZindex($(this.target)) + 1);
                $datepicker._datepickerShowing = true;
        ...
        cleanUp: function (rollback) {
            ...
            if (rollback) {
                var $selection = $(this.editor.dom.select('span#datepicker')),
                    replacement = $('<p>' + this.options.trigger + '</p>')[0].firstChild,
        ...
        offset: function () {
            var rtePosition = $(this.editor.getContainer()).offset(),
                ...
                nodePosition = $(this.editor.dom.select('span#datepicker')).position();
        ...
        offsetInline: function () {
            var nodePosition = $(this.editor.dom.select('span#datepicker')).offset();
        ...
        rteClicked: function (e) {
            ...
            if (this.hasFocus && $target.parent().attr('id') !== 'datepicker-datetext') {
                this.cleanUp(true);
            }
        },
    ...
    tinymce.create('tinymce.plugins.DatePicker', {
        ...
        init: function (editor) {
            ...
            $datepicker.editor = editor;
            
            var datePicker,
                datePickerOptions = editor.getParam('datepicker') || {},
                expectedTrigger = datePickerOptions.trigger || '#';
            
            $datepicker.options = $.extend({}, $datepicker.options, datePickerOptions);
    ...
    tinymce.PluginManager.add('datepicker', tinymce.plugins.DatePicker);
